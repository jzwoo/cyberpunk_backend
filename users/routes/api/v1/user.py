...
async def get_user(user_id: str, requester=Depends(verify_token)):
    return get_user_controller(connection_db, user_id, requester)
...
async def update_user(user_id: str, user_update: UserUpdate, requester=Depends(verify_token)):
    return update_user_controller(connection_db, user_id, user_update, requester)


@user.delete('/api/v1/users/{user_id}', response_description="Delete existing user")
async def delete_user(user_id: str, requester=Depends(verify_token)):
    return delete_user_controller(connection_db, user_id, requester)
